...
TScene loadBinaryScene(char* fileName, int index)
{
   ...
   if(scenePtr)
   {
        ...
        Scene.CubeEntry = *(scenePtr); scenePtr+=5;
        ...
        Scene.Hero.lifeScript = decompLifeScript(scenePtr, heroLifeBytes, heroTrack);
        ...
        for(int i=1; i < Scene.numActors; i++)
        {
            ...
            Actor.lifeScript = decompLifeScript(scenePtr, lifeBytes, actorTrack);
            scenePtr += lifeBytes;
    ...
    *ptr = 0;
    ...
    do
    {
       ...
       switch (opcode)
       {
           ...
           {
               sprintf(scriptbuff,"WAIT_NB_ANIM %d",*(scriptPtr++));
               scriptPtr++; // dummy
               break;
           }
           case 14: // SAMPLE
...
char * decompLifeScript(unsigned char *scriptPtr, int lifeBytes, unsigned char *trackPtr)
{
...
void saveBinaryScene(TScene Scene, char* fileName)
{
   ...
   compTrack(Scene.Hero.trackScript, sceneHandle);
...
*/
   ...
   for(int i=1; i < Scene.numActors; i++)
   {
       ...
       compTrack(Scene.Actors[i-1].trackScript, sceneHandle);
...
void resolveCompOffsets(TScene Scene)
{
  ...
  {
    compTrackOffsets(Scene.Hero.trackScript, a);
    compLifeOffsets(Scene.Hero.lifeScript, a);
  }
}

void compTrack(char * script, FILE* sceneHandle)
{
    int currentOffset=0;
    int finish=0;
    int line=0;
            ...
            case 32: // SIMPLE_SAMPLE
                {
                    ...
                    *(track.script+currentOffset) = (unsigned char)opcode;
                    *(track.script+currentOffset+1) = (unsigned char)value & 0x00FF;
                    *(track.script+currentOffset+2) = ((unsigned char)value & 0xFF00)>>2;
             ...
             case 34: // ANGLE_RND
                {
                    ...
                    *(track.script+currentOffset) = (unsigned char)opcode;
                    *(track.script+currentOffset+1) = (short int)value1;
                    *(track.script+currentOffset+3) = (short int)value2;
             ...
             case 30: // PLAY_FLA      <- RECHECK
                {
                    ...
                    track.script=(unsigned char*)realloc((unsigned char*)track.script,currentOffset+1+strlen(value));

                    *(track.script+currentOffset) = (unsigned char)opcode;
                    memcpy(track.script+currentOffset+1,&value,strlen(value));

                    currentOffset=currentOffset+1+strlen(value);
                    break;
                }
        }
            ...
            case 17: // BACKGROUND
                {
                    if(opcode==9) // LABEL
                    {
                      trackOffsets[actor][currentTrack] = currentOffset;
                      currentTrack++;
                    }
                    currentOffset=currentOffset+2;
                    break;
            ...
            case 103: // TEXT  
                {
                    ...
                    *(life.script+currentOffset) = (unsigned char)opcode;
                    *(life.script+currentOffset+1) = (unsigned char)value & 0x00FF;
                    *(life.script+currentOffset+2) = ((unsigned char)value & 0xFF00)>>2;
            ...
            case 64: // PLAY_FLA      <- RECHECK
                {
                    ...
                    life.script=(unsigned char*)realloc((unsigned char*)life.script,currentOffset+1+strlen(value));

                    *(life.script+currentOffset) = (unsigned char)opcode;
                    memcpy(life.script+currentOffset+1,&value,strlen(value));
             ...
             case 91: // ASK_CHOICE_OBJ
                {
                    ...
                    *(life.script+currentOffset) = (unsigned char)opcode;
                    *(life.script+currentOffset+1) = (unsigned char)value1;
                    *(life.script+currentOffset+2) = (short int)value2;
